...
void               gmpd_status_set_partition         (GMpdStatus        *self,
                                                      const gchar       *partition);

void               gmpd_status_set_volume            (GMpdStatus        *self,
                                                      gint8              volume);
...
void               gmpd_status_set_queue_length      (GMpdStatus        *self,
                                                      guint              queue_length);

void               gmpd_status_set_playback          (GMpdStatus        *self,
                                                      GMpdPlaybackState  playback);

void               gmpd_status_set_current_position  (GMpdStatus        *self,
                                                      guint              current_position);

void               gmpd_status_set_current_id        (GMpdStatus        *self,
                                                      guint              current_id);

void               gmpd_status_set_next_position     (GMpdStatus        *self,
                                                      guint              next_position);

void               gmpd_status_set_next_id           (GMpdStatus        *self,
                                                      guint              next_id);

void               gmpd_status_set_current_elapsed   (GMpdStatus        *self,
                                                      gfloat             current_elapsed);

void               gmpd_status_set_current_duration  (GMpdStatus        *self,
                                                      gfloat             current_duration);

void               gmpd_status_set_bit_rate          (GMpdStatus        *self,
                                                      guint              bit_rate);

void               gmpd_status_set_crossfade         (GMpdStatus        *self,
                                                      guint              crossfade);

void               gmpd_status_set_mixramp_db        (GMpdStatus        *self,
                                                      gfloat             mixramp_db);

void               gmpd_status_set_mixramp_delay     (GMpdStatus        *self,
                                                      gfloat             mixramp_delay);

void               gmpd_status_set_audio_format      (GMpdStatus        *self,
                                                      GMpdAudioFormat   *audio_format);

void               gmpd_status_set_db_update_job_id  (GMpdStatus        *self,
                                                      guint              db_update_job_id);

void               gmpd_status_set_error             (GMpdStatus        *self,
                                                      const gchar       *error);
